...
//  Birthday Formatting
//
//  Created by Quentin Rubini on 13/04/2015.
...
- (BOOL) textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (activeLength == 0 &&
        self.text.length == 10 &&
        ![self.text isEqualToString:__placeholderText])
    {
        activeLength = self.text.length;
    }
        ...
        if (string.length <= 0)
        {
            if (textField.text.length > 0)
            {
                ...
                if (activeLength <= deleteDelta)
                {
                    ...
                    [self setText:[NSString stringWithFormat:@"%@", [self.text substringToIndex:(activeLength - deleteDelta)]]];
            }
        }
        else if ([string rangeOfCharacterFromSet:__nonNumbersSet].location == NSNotFound)
        {
                ...
                if (month <= 12 && month >= 0)
                {
                    if (textField.text.length == 0)
                    {
                        ...
                        if ([string integerValue] > 1)
                        {
                            ...
                            //----
                            [self setText:[NSString stringWithFormat:@"0%@/", string]];
                        }
                        else
                            [self setText:[NSString stringWithFormat:@"%@", string]];
                    }
                    else
                    {
                        //-----
            ...
            else if (activeLength < 6)
            {
                ...
                if (activeLength == 3)
                {
                    ...
                    {
                        [self setText:[NSString stringWithFormat:@"%@%@", [self.text substringToIndex:activeLength], string]];
                    }
                    else
                    {
                        [self setText:[NSString stringWithFormat:@"%@0%@/", [self.text substringToIndex:activeLength], string]];
                    }
                }
